$(function() {
    ...
    /* ==================================================
       4. NEW SECTION (new_selected 적용)
       ================================================== */
    (function() {
        const $inner = $('.new_item_inner');
        const $dots = $('.new_container .page_pointer .page');
        let isDrag = false, startX = 0, currentX = 0, pageIdx = 0;
        const pageCount = $('.new_container .item_sec').length;

        function updatePos(ani = true) {
            const moveX = -(pageIdx * 100);
            $inner.css({ 
                'transition': ani ? 'margin-left 0.4s ease' : 'none', 
                'margin-left': moveX + 'vw' 
            });
            // new_selected 적용
            $dots.removeClass('new_selected').attr('src', 'images/dot_w.png');
            $dots.eq(pageIdx).addClass('new_selected').attr('src', 'images/dot_b.png');
        }
        ...
        $(window).on('mousemove touchmove', function(e) {
            ...
            const movePx = -(pageIdx * $(window).width()) + (currentX - startX);
        ...
        $(window).on('mouseup touchend', function() {
            ...
            if (diff < -50 && pageIdx < pageCount - 1) pageIdx++;
            else if (diff > 50 && pageIdx > 0) pageIdx--;
            updatePos();
        });
    ...
    /* ==================================================
       5. NEWS SECTION (arit_selected 적용)
       ================================================== */
    (function() {
        const $inner = $('.news_inner');
        const $dots = $('.news_wrapper .page_pointer .page');
        let isDrag = false, startX = 0, currentX = 0, pageIdx = 0;
        const pageCount = $('.news_wrapper .news_card').length;
        ...
        function updatePos(ani = true) {
            ...
            // arit_selected 적용
            $dots.removeClass('arit_selected').attr('src', 'images/dot_w.png');
            $dots.eq(pageIdx).addClass('arit_selected').attr('src', 'images/dot_b.png');
        }
